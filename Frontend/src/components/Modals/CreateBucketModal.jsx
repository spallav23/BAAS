...
import { createBucket, fetchBuckets } from '../../store/slices/storageSlice'
...
const CreateBucketModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(createBucket(formData)).unwrap()
      toast.success('Bucket created successfully!')
      dispatch(fetchBuckets())
      onClose()
    } catch (error) {
      toast.error(error || 'Failed to create bucket')
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-dark-border">
            <h2 className="text-xl font-bold text-dark-text">Create Bucket</h2>
            <button
              onClick={onClose}
