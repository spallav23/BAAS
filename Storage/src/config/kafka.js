...
const initKafka = () => {
  ...
  consumer = kafka.consumer({ groupId: process.env.KAFKA_GROUP_ID || 'storage-service-group' });
...
const connectKafka = async () => {
  try {
    ...
    await consumer.subscribe({ topics: ['user-events', 'storage-events'], fromBeginning: false });
...
const publishEvent = async (topic, event) => {
  try {
    ...
    await producer.send({
      ...
      messages: [
        {
          key: event.bucketId || event.userId || 'default',
